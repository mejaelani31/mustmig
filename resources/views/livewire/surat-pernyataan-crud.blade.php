...
<div>
            ...
            @foreach($items as $item)
                <tr>
                    ...
                    <td class="px-3 py-2 text-sm text-gray-900 dark:text-gray-100">
                        <x-secondary-button wire:click="show({{ $item->id }})" class="mr-2">Show</x-secondary-button>
    ...
    <x-dialog-modal wire:model.live="showingShowModal">
        <x-slot name="title">{{ __('Detail Surat Pernyataan') }}</x-slot>

        <x-slot name="content">
            <div class="grid grid-cols-1 gap-2 text-sm text-gray-700 dark:text-gray-300">
                <div><span class="font-semibold">ID Pelanggan:</span> {{ $currentSurat->id_pelanggan ?? '' }}</div>
                <div><span class="font-semibold">Nama Pelanggan:</span> {{ $currentSurat->nama_pelanggan ?? '' }}</div>
                <div><span class="font-semibold">Alamat Pelanggan:</span> {{ $currentSurat->alamat_pelanggan ?? '' }}</div>
                <div><span class="font-semibold">Tarif:</span> {{ $currentSurat->tarif_pelanggan ?? '' }}</div>
                <div><span class="font-semibold">Daya:</span> {{ $currentSurat->daya_pelanggan ?? '' }}</div>
                <div><span class="font-semibold">Tanggal:</span> {{ $currentSurat->tanggal_ttd ?? '' }}</div>
                <div><span class="font-semibold">Nama Penanda Tangan:</span> {{ $currentSurat->nama_ttd ?? '' }}</div>
                <div><span class="font-semibold">Alamat Penanda Tangan:</span> {{ $currentSurat->alamat_ttd ?? '' }}</div>
                <div><span class="font-semibold">NIK:</span> {{ $currentSurat->nik_ttd ?? '' }}</div>
                <div><span class="font-semibold">No HP:</span> {{ $currentSurat->nohp_ttd ?? '' }}</div>
                <div><span class="font-semibold">File:</span>
                    @if($currentSurat?->file_ttd)
                        <a href="{{ Storage::disk('public')->url($currentSurat->file_ttd) }}" target="_blank" class="text-indigo-600 underline">{{ __('Lihat PDF') }}</a>
                    @endif
                </div>
            </div>
        </x-slot>
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="closeShowModal">{{ __('Close') }}</x-secondary-button>
        </x-slot>
    </x-dialog-modal>
</div>
